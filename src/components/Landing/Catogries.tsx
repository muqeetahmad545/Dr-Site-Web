...
const Categories = () => {
    ...
    return (
        <div className="flex justify-center bg-cover bg-no-repeat w-full" style={{ backgroundImage: "url('https://res.cloudinary.com/dfizbmrep/image/upload/v1710497213/categorybg_m2n3ne.png')" }}>
                ...
                <div className='flex justify-center'>
                    <div className='grid grid-cols-4 max-sm:grid-cols-1 max-lg:grid-cols-2 max-xl:grid-cols-3 items-center max-sm:gap-6 gap-16 mb-32 w-[80%] justify-center'>
                        {categories.map((category, index) => (
                            <div key={index} className='py-8 max-w-80 px-4 border relative border-[#c5c5c5] max-sm:m-auto max-md:m-auto rounded-xl h-full hover:text-white hover:bg-secondary hover:border-none transition-all duration-300  hover:shadow-md' >
                                <div className='w-[75%] flex flex-col gap-5 mb-8 max-sm:w-full'>
                                    <div className={`w-[70px] h-[75px] rounded-2xl flex items-center justify-center`} style={{ backgroundColor: category.bgColor }}>
                                        <img src={category.imagePath} alt='icon' width={35} height={34} />
                                    </div>
                                    ...
                                    </div>
                                    {
                                        ...
                                        <img src={category.additionalImagePath} alt='icon' width={112} height={112} className='absolute -top-14 right-0' />
                                    }
                                </div>
                            </div>
                        ))}
